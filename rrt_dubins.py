...
class Dubins(object):
    ...
    def dubinsLSR(self, d, alpha, beta):
        ...
        if (tmp > self.constant["dubins_zero"]):
            p = np.sqrt(np.amax([tmp, 0]))
            theta = np.arctan2(-ca - cb, d + sa + sb)
            theta2 = np.arctan2(-2, p)
            t = twopify(-alpha + theta - theta2)
            q = twopify(-beta + theta - theta2)
    ...
    def get_best_dubins_path(self, d, alpha, beta):
        dubins_functions = [
            self.dubinsLSL, self.dubinsLRL, self.dubinsRSR, self.dubinsRLR, self.dubinsLSR, self.dubinsRSL
        ]
